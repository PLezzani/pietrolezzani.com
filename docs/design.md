...
The bar carries three things at every width: the name, the way to the rest of the site, and the one action. Below 720px the two page links move out of it and behind a `nav-toggle`, a burger at the far right, and open in a `site-nav-panel` that hangs under the bar and travels with it. The action never follows them into the panel: it is the only thing the site asks for and it stays in sight on the narrowest screen. The panel closes on Escape, on a tap outside it, on the link that was just followed, and on the first scroll, because the bar slides away on the way down and would carry an open menu with it. The bars cross into a close sign on the same press that opens the panel, which is feedback on a control rather than an arrival, and the one kind of motion that may happen every time it is touched. Without script the three items are simply side by side in the bar, where they fit down to 320px through two steps of size, and no burger is drawn that nothing could open.
...
`section-invert` is the second and last ground change, and it is spent on the section that holds the only action: `Start with a conversation, not a brief` sits on cream, in black, with the plan boxes bordered in `border-inverse` and the button turned over, black fill and cream label, the accent still arriving from where the cursor crossed the edge. A change of ground is the loudest device this system has, louder than any size of type, so it goes where a reader decides whether to book the call rather than anywhere it would merely look good. The hairline between sections is dropped there, since the ground already separates them. On a case page the action shares its section with the outcome numbers, so it stays on black: inverting it would light up the figures next to it rather than the action.

`footer` keeps the page's own ground. It inverted it for as long as it was the last light block on the page; with the action on cream, a cream footer under it reads as one block with the action, and the quietest part of the page ends up as loud as the loudest. So the page closes on black, the footer set apart by a hairline and by the air above it. It carries the name in `display-m`, the role and city under it, contact links in `body-l` underlined with `divider`, and a legal line below a rule, every line at weight 400: what is here is reference, not argument. It holds no call to action. The one action already appears three times above it, and a fourth in the footer would turn a close into a nag.
...
Sections follow one order without exception: `section-label`, then the title, then the content. A label that appears below the title it labels reads as a caption for whatever precedes it, which is never what was meant. Nothing is drawn between the label and the title: the label is already set apart by size, colour and its accent square, and a line there closes it off from the sentence it introduces, like a lid on the section. The title opens `lg` below the label in every section, list or not, the same distance at which the home's three cards open under theirs, and that repeated distance is what makes four different sections read as one page: a reader who has learned where a section starts should never have to relearn it. Hairlines are kept for what they actually separate, one row from the next and one section from the next.
