...
`affiliation` names a network or a body the practice belongs to. It sits under a hairline in the title column: the name in `body-l`, then the organisation's logo. A borrowed logo is admissible here on one condition, that it is a single colour and that colour is `on-surface`, so it adds no hue the palette does not already carry. That is a rule about affiliation rather than about logos: a mark in this block is a claim the practice makes about itself, and a claim does not get to arrive wearing someone else's brand colours. A client's mark inside `case-thumb` is a different object, an identification of whose case sits behind a closed row rather than a claim about the practice, and it keeps its own colours, next to covers that are full colour already. Keep it near 180px wide, well under the display type above it, because the affiliation supports the claim rather than making it.
